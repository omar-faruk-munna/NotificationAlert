...
namespace NasWorker
{
    public class Worker : BackgroundService
    {
        ...
        private readonly int _eventTryPeriod;
        ...
        private readonly string _isEventStart;
        ...
        public Worker(ILogger<Worker> logger, IConfiguration configuration, IHttpClientFactory clientFactory)
        {
            ...
            _eventTryPeriod = _configuration.GetValue<int>("EventTryPeriod");
            _alertTryPeriod = _configuration.GetValue<int>("AlertTryPeriod");
            _numberPrefix = _configuration.GetValue<string>("NumberPrefix");
            _isEventStart = _configuration.GetValue<string>("IsEventStart");
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            try
            {
                eventTimer = new Timer(new TimerCallback(TickTimer1), null, 0, _eventTryPeriod);
        ...
        private void ExecuteNasEvent()
        {
            try
            {
                ...
                con.Open();
                IEnumerable<dynamic> result = con.QueryAsync<dynamic>(fncRecGetQuery, dynamicParameters, commandType: CommandType.StoredProcedure).Result;
                ...
                foreach (var rows in result)
                {
                    ...
                    OracleDynamicParameters eventParameters = new OracleDynamicParameters();
                    eventParameters.Add("pauth_status_id", "A", OracleDbType.NVarchar2, ParameterDirection.Input);

                    int msgFncId = Convert.ToInt32(fields["MSG_FNC_ID"]);
                    int msgTypeId = Convert.ToInt32(fields["MSG_TYP_ID"]);
                    ...
                    //string fncStart = Convert.ToString(fields["FNC_START"]);
                    //string fncEnd = Convert.ToString(fields["FNC_END"]);

                    DateTime today = DateTime.Now;
                    DateTime eventStart = DateTime.Now;
                    DateTime eventEnd = DateTime.Now;

                    //if (!string.IsNullOrEmpty(fncStart))
                    //{
                    //    eventStart = DateTime.ParseExact(fncStart, "HH:mm:ss", CultureInfo.InvariantCulture);
                    //}

                    //if (!string.IsNullOrEmpty(fncEnd))
                    //{
                    //    eventEnd = DateTime.ParseExact(fncEnd, "HH:mm:ss", CultureInfo.InvariantCulture);
                    //}

                    int sinceLastExec = today.Subtract(lastExecution).Days;
                    OracleDynamicParameters parameters = new OracleDynamicParameters();
                    parameters.Add("pmsg_fnc_id", msgFncId, OracleDbType.Int32, ParameterDirection.Input);
                    parameters.Add("pprocess_last_exec", today.ToString("MM/dd/yyyy hh:mm:ss tt"), OracleDbType.NVarchar2, ParameterDirection.Input);

                    //if (string.IsNullOrEmpty(fncStart) && string.IsNullOrEmpty(fncEnd))
                    //{
                    if (msgTypeId.Equals(2) && frequency.Equals("D") && sinceLastExec >= 1) //Daily-2-D
                    {
                        try
                        {
                            result = con.QueryAsync(processName, eventParameters, commandType: CommandType.StoredProcedure).Result;

                            con.QueryAsync(setExeQuery, parameters, commandType: CommandType.StoredProcedure).Wait(10);
                        }
                        catch (Exception e)
                        {
                            LogError("Exeption From Daily Section.");
                            LogError(e.Message);
                        }
                    }
                    else if (msgTypeId.Equals(3) && frequency.Equals("M") && today.Day >= 1 && sinceLastExec >= 1) //Monthly-3-M
                    {
                        try
                        {
                            result = con.QueryAsync(processName, eventParameters, commandType: CommandType.StoredProcedure).Result;
                        ...
                        {
                            LogError("Exeption From Monthly Section.");
                            LogError(e.Message);
                        }
                    }
                    else if (msgTypeId.Equals(1) && frequency.Equals("T")) //Timely-1-T
                    {
                        try
                        {
                            Task.Run(() =>
                        ...
                        {
                            LogError("Exeption From Secondly Section.");
                            LogError(e.Message);
                        }
                    }

                    //}
                    //else if (eventStart.TimeOfDay <= DateTime.Now.TimeOfDay && eventEnd.TimeOfDay >= DateTime.Now.TimeOfDay)
                    //{
                    //    if (msgTypeId.Equals(2) && frequency.Equals("D") && sinceLastExec >= 1) //Daily-2-D
                    //    {
                    //        try
